	...
	public class ReturnValue  {
		...
		public string FromNative (string var)
		{
			...
			if (ElementType != String.Empty) {
				...
				return String.Format ("({0}) Gst.GLib.Marshaller.ArrayPtrToArray ({1}, typeof ({2}), (int){3}native_{4}, true)", CSType, var, IGen.QualifiedName, CountParameter.CSType == "int" ? String.Empty : "(" + CountParameter.CSType + ")", CountParameter.Name);
			else
		...
		public string ToNative (string var)
		{
			...
			if (ElementType.Length > 0) {
				...
				return String.Format ("Gst.GLib.Marshaller.ArrayToArrayPtr ({0})", var);
		...
		public bool Validate (LogWriter log)
		{
			if (MarshalType == "" || CSType == "") {
				...
				return false;
			} else if ((CSType == "Gst.GLib.List" || CSType == "Gst.GLib.SList") && String.IsNullOrEmpty (ElementType))
